...
export const DB = {
  ...
  saveMessage: (sessionId: string, role: string, content: string, images?: string[]) => {
    const stmt = db.prepare('INSERT INTO messages (sessionId, role, content, createdAt, images) VALUES (?, ?, ?, ?, ?)');
    stmt.run(sessionId, role, content, Date.now(), images ? JSON.stringify(images) : null);
  },

  getMessages: (sessionId: string) => {
    const rows: any[] = db.prepare('SELECT role, content, images FROM messages WHERE sessionId = ? ORDER BY createdAt ASC').all(sessionId);
    return rows.map(r => ({
      ...r,
